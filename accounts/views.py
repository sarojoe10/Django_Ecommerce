...
class CustomTokenGenerator(PasswordResetTokenGenerator):
    def _make_hash_value(self, user, timestamp):
        ...
        return f"{user.pk}{user.is_active}{user.password}{last_login}{timestamp}"
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            message = render_to_string('accounts/forgotPassword_mail.html',{
                'user':user,
                'domain': current_site,
                'uid' : urlsafe_base64_encode(force_bytes(user.pk)),
                'token':custom_token_generator.make_token(user) 
            ...
            messages.success(request, 'Password reset link has been sent to your email address {{email}}')
            return redirect('login')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            messages.error('Password doesnot match')
            return redirect('resetPassword')
    else:
